...
**Architecture:** pyslang-based AST analyzer extracts package/module/instantiation dependencies per file → topological sort determines compile order → IcarusBackend verifies with `eda-env iverilog -g2012 -o /dev/null -f <f>` → AIRepairLoop invokes Claude API on failure up to `--max-iter` times. SimulatorBackend is an ABC so swapping to Xcelium only requires a new subclass.

**Tech Stack:** Python 3.12, pyslang (SV AST), anthropic SDK (Claude API), pyyaml (macro dump), click (CLI), pytest (TDD), `eda-env iverilog` (Icarus Verilog 12.0)
...
- `src/vcodeman/gen/analyzer.py` — pyslang AST + regex → `FileInfo` per file
...
**Modified files:**
- `pyproject.toml` — add `pyslang`, `anthropic`, `pyyaml` dependencies
...
## Task 3: Analyzer (pyslang AST + regex)
...
SK = pyslang.SyntaxKind
TK = pyslang.TokenKind

_INCLUDE_RE = re.compile(r'`include\s+"([^"]+)"')
_DEFINE_RE = re.compile(r'`define\s+(\w+)(?:\s+(.+))?')
_IFDEF_RE = re.compile(r'`(?:ifdef|ifndef)\s+(\w+)')
...
def _find_all(node, kind: object) -> list:
    results = []
    if node.kind == kind:
        results.append(node)
    try:
...
def _first_ident(node) -> str | None:
    """Return valueText of the first Identifier token among direct children."""
    try:
        for child in node:
            if child.kind == TK.Identifier:
...
def analyze_file(path: Path) -> FileInfo:
    """Parse one RTL file and extract all dependency-relevant information."""
    path = path.resolve()
    info = FileInfo(path=path)

    raw = path.read_text(encoding="utf-8", errors="replace")

    # --- regex pass (preprocessor directives not visible in pyslang AST) ---
    for m in _INCLUDE_RE.finditer(raw):
        info.included_files.append(m.group(1))

    for i, line in enumerate(raw.splitlines(), start=1):
        m = _DEFINE_RE.match(line.strip())
        if m:
            info.defined_macros.append(
                MacroDef(name=m.group(1), value=m.group(2), defined_in=path, line=i)
            )

    for m in _IFDEF_RE.finditer(raw):
        name = m.group(1)
    ...
    # --- pyslang AST pass ---
    try:
        tree = pyslang.SyntaxTree.fromText(raw)
        root = tree.root

        for node in _find_all(root, SK.PackageDeclaration):
            headers = _find_all(node, SK.PackageHeader)
            if headers:
                name = _first_ident(headers[0])
                if name:
                    info.declared_packages.append(name)

        for node in _find_all(root, SK.ModuleDeclaration):
            mod_headers = _find_all(node, SK.ModuleHeader)
            if mod_headers:
                name = _first_ident(mod_headers[0])
                if name:
                    info.declared_modules.append(name)
            for imp in _find_all(node, SK.PackageImportItem):
                pkg = _first_ident(imp)
                if pkg and pkg not in info.imported_packages:
                    info.imported_packages.append(pkg)
            for inst in _find_all(node, SK.HierarchyInstantiation):
                mod_type = _first_ident(inst)
                if mod_type and mod_type not in info.instantiated_modules:
                    info.instantiated_modules.append(mod_type)

    except Exception:
        pass  # parse errors: partial info is still useful
...
git commit -m "feat(gen): analyzer — pyslang AST + regex dependency extractor"
```
